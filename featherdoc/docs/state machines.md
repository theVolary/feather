...
Let us start with a simplier example first to mainly demonstrate the syntax and structure for creating states and events.  Also it is important to show how transitions are generated in feather state machines.  The below example is just 3 states and two transitions, but it really touches on some of the basics that I believe are important.
