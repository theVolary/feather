...
if [ "$1" == "--help" ]; then
  ...
  node lib/node-jsdoc-toolkit/app/run.js --exclude="date\.js$" --exclude="jquery.*\.js$" --exclude=".*test.*" --exclude="socket\.io\.client" --exclude="node-htmlparser" --exclude="feather-client" --verbose --multiple --recurse  --directory=public/docs/api/server --template=lib/node-jsdoc-toolkit/templates/codeview ../lib/node-core-enhancements ../lib  
