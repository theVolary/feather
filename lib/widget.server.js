...
jojo.widget.registerTagHandler("async", function(options) {
  var tmpl = $j(options.tag).html();
  asyncTmplExpressions.each(function(expr) {
    tmpl = tmpl.replace(expr.expression, expr.replace);
  });
  var method = options.tag.getAttribute("method");
  var placeholder = $j("<placeholder/>").insertBefore(options.tag);
  $j(options.tag).remove();
  if (typeof options.widget[method] === "function") {
