  ...
  rendering: {
    stateStartup: function(widget, args) {
      if (widget && widget.container && args) {
        ...
        var html = $j.tmpl(widget.template, widget).html();
        ...
        if (clientScriptItem) {
          ...
          $j("<script type='text/javascript'>" + onInitStr + "</script>").appendTo(body);
        }
        
        //now render any children widgets that just got embedded
        jojo.widget.render(widget.container[0], widget.id + "_", widget);
      }
    ...
    widgets && widgets.each(function(index, widget) {  
      ...
      if (instance.getClientScript) {
        ...
        if (clientScript && clientScript !== "") {
          ...
          $j("<script type='text/javascript'>" + clientScript + "</script>").appendTo(body);
        }
      }
