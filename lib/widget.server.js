 ...
 * rendering process.
 */
jojo.widget.render = function(node, idPrefix, parentWidget, level) {
  level = level || 0;
  $j("widget:first", node).each(function(index, widget) {  
    ...
    if (instance.getClientScript) {
      ...
      if (clientScript && clientScript !== "") {
        ...
        $j("<clientscript type='text/javascript' order='a' level='" + level.toString() + "'>" + clientScript + "</clientscript>").appendTo(body);
      }
    }
        
    //recurse down the children tree first so rendering goes from the bottom up
    jojo.widget.render(widget, id + "_", instance, level++);
  ...
  if ($j("widget:first", node).length > 0) { //need an exit condition to prevent infinite recursion
    jojo.widget.render(node, idPrefix, parentWidget, level);
  }
};
