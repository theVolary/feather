...
jojo.widget.prototype.renderCss = function() {
  var me = this;
  
  if (me.clientCssHrefPath && me.clientCssFilePath) {
  
    var widgetCssExists = jojo.cssFiles[me.clientCssFilePath];
    if (widgetCssExists) {
      
      if (! jojo.request.loadedCss) {
        jojo.request.loadedCss = {};
      }
      
      if (! jojo.request.loadedCss[me.clientCssFilePath]) {      
       
        sys.puts("Loading css for " + me.clientCssFilePath)
        jojo.request.loadedCss[me.clientCssFilePath] = true;
        
        // If we're in debug mode, go ahead and append the <link>.  Otherwise, 
        // we'll use the loadedCss hash at the end of the request to generate
        // one css file and append its <link>.
        if (jojo.debug) {
          var head = $j('head');
          $j.tmpl(widgetCssTemplate, { href : me.clientCssHrefPath}).appendTo(head);
        }
      }
    }
  }
