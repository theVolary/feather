 ...
 */
feather.widget.widgets.on("beforeWidgetClassCreation", function(args) {
  var serverMethods = [];
  ...
  if (serverMethods.length > 0) {
    ...
    prototype.getClientScript = function($super, options) {
      ...
      var key = feather.request.page + prototype.widgetPath + "_glueEmitted";
      if (clientScript != "" && !feather.widget.widgets[key]) {
        serverMethods.each(function(p){
          ...
          var tmpl = feather.dom.$j.tmpl(glueTemplate, replacements);
          clientScript = tmpl.html() + "\\n" + clientScript + "\\n";
        });
        feather.widget.widgets[key] = true;
      }
      return clientScript;
    };
  }
...
feather.widget.prototype.getClientScript = function(options) {
  ...
  var parentStr = me.parent ? "feather.lang.objects['" + me.parent.id + "']" : "null";
  var replacements = {
  ...
  var tmpl = feather.dom.$j.tmpl(_instanceScript || instanceScript, replacements);
  clientScript += "\\n" + tmpl.html();
  return clientScript;
};
...
feather.widget.registerTagHandler("async", function(options) {
  ...
  if (!asyncMethods[methodName]) {
    asyncMethods[methodName] = feather.ns(methodName, global, true);
  }
  if (typeof asyncMethods[methodName] === "function") {
    options.renderOptions.result.renderers.push(function(renderOptions, cb) {
      (asyncMethods[methodName])(function(result) {
        var newStr = _t(feather.dom.$j, {
          data: result
        }).join("");     
