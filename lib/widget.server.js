 ...
 */
jojo.widget.registerTagHandler("template", function(options) {
  var $j = options.dom.$j;
  ...
  options.widget.templates = options.widget.templates || new jojo.lang.registry();
  var template = {
    id: name,
    tmpl: tmpl
  };
  ...
  if (client_enabled == "true") {
    ...
    if (options.widget.templates.items.length == 1) {
      scriptBuilder.unshift('widget.templates = widget.templates || new jojo.lang.registry();');
    }
    options.widget.scripts.push(scriptBuilder.join("\\n"));
  }
});
jojo.widget.registerTagHandler("insert_template", function(options) {
  var $j = options.dom.$j;
  var name = options.tag.getAttribute("name");
