  ...
  rendering: {
    stateStartup: function(widget, args) {
      if (widget && widget.container && args) {
        ...
        //TODO: evaluate this API (not sure i like the 'async' tag name, but just going with it for now to get functionality working)
        //find any async blocks and insert placeholders as well as setup the callbacks
        $j("async", widget.container).each(function(index, asyncTag) {
          var tmpl = $j(asyncTag).html();
          asyncTmplExpressions.each(function(expr) {
            tmpl = tmpl.replace(expr.expression, expr.replace);
          });
          var method = asyncTag.getAttribute("method");
          var placeholder = $j("<placeholder/>").insertBefore(asyncTag);
          $j(asyncTag).remove();
          if (typeof widget[method] === "function") {
            args.renderFSM.suppress("ready", true);
            args.asyncCount++;
            widget[method](function(result) {
              $j.tmpl(tmpl, result).insertBefore(placeholder);
              placeholder.remove();
              args.asyncCount--;
              if (args.asyncCount == 0) {
                args.renderFSM.unsuppress("ready");
              }
            });
          } else {
            jojo.logger.error("no async method '" + method + "' found.");
            placeholder.remove();
          }
        });
...
/**
 * static states object to re-used for the renderFSM instances
 */
var renderFSMStates = {
  states: {
    initial: {
...
jojo.widget.render = function(renderOptions, callback) {
  if (typeof callback === "function") {
    var _fsm = jojo.widget.render(renderOptions);
    _fsm.onceState("ready", callback);
    //this will happen immediately if no async blocks are pending, otherwise will happen when all async blocks are finished
    _fsm.fire("ready"); 
    return _fsm;
  }
  ...
  renderOptions.renderFSM = renderOptions.renderFSM || new jojo.fsm.finiteStateMachine(renderFSMStates);
  var renderFSM = renderOptions.renderFSM;
