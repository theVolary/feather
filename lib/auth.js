    ...
    if (!sessionId) {
      ...
      cache.getItemsWait([
        ...
        if (err) cb(err); else {
          ...
          sessionId = featherUtil.parseSignedCookie(sessionId, appOptions.connect.session.secret);
          authdb.get({id: appOptions.auth.userIdPrefix + username}, function(err, dbResult) {
            if (!err) {
