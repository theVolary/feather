...
var feather = module.exports = /** @lends feather */ {
  ...
  /**
   * Parse a signed session cookie (id) using the configured connect session secret.
   */
    ...
    feather.dom = new DomResource({
      onState: {
        ready: function() {
           ...
           * @name feather.domManager
           * @see DomManager
           */
          feather.domManager = new DomManager();
          cache.setItem("feather-domManager", feather.domManager);
              ...
              if (err) cb(err); else {
                ...
                if (options.isWorker) { //only setup actual server stuff when in a worker
                  ...
                  if (options["socket.io"].enabled) {
                    socket.init(options, function(err, socketServers) {
                      if (err) throw err;

                      feather.socketServers = socketServers;
                      cache.setItem("feather-socketServers", socketServers);
  ...
  shutdown: function(code) {
    ...
    if (me.heapTimer) { // clean up, clean up, everybody clean up!
      clearInterval(me.heapTimer);
      delete me.heapTimer;
    }
  ...
  lang: {
    ...
    Promise: Promise,
    /**
     * Generic class for pooling resources
     */
