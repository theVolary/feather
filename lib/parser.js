 ...
 */
feather.parser.parseFile = function(options, cb) {
  ...
  fs.readFile(path, "utf-8", function(err, data) {
    ...
    feather.domPool.getResource(function(dom) {      
      ...
      feather.widget.render({
        ...
        node: dom.document,
        request: req,
        publicRoot: feather.appOptions.publicRoot
      }, function(result) {
        ...
        ResourcePackager.packageFrameworkResources({
          ...
          request: req,
          publicRoot: feather.appOptions.publicRoot,
          resources: feather.appOptions.resources,
          featherRoot: feather.appOptions.featherRoot,
          auth: feather.appOptions.auth,
          data: feather.appOptions.data,
          ui: feather.appOptions.ui,
          widgetClassRegistry: feather.appOptions.widgetClassRegistry
        });      
...
feather.parser.parseWidget = function(options, cb) {
  feather.domPool.getResource(function(dom) {
    ...
    var widgetId = options.id || feather.id();
    var safeWidgetId = feather.id() + "___" + widgetId; //3 underscores just to make the replaces below more accurate
