...
var publisher = module.exports = function(options) {
  publisher.super.apply(this, arguments); //get our minimal base class stuff (i.e. id, dispose)
  process.EventEmitter.apply(this); //get the base node EventEmitter functionality
  ...
  if (config && (config.on || config.once)) {
    ...
    if (config.once) {
      for (var evt in config.once) {
        this.once(evt, config.once[evt]);
      }
    }
  }
};

_.extend(publisher.prototype, process.EventEmitter.prototype)

var _emit = publisher.prototype.emit;
publisher.prototype.emit = publisher.prototype.fire = function (type, args) {
...
publisher.prototype.dispose = function() {
  ...
  publisher.super.prototype.dispose.apply(this, arguments);
};

inherits(publisher, base);