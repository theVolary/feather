...
function _Logger(options) {
  ...
  _Logger.super.apply(this, arguments);
};
...
inherits(_Logger, EventPublisher);
...
var fileAppender = function(options) {
    ...
    var appender = function(loggingEvent) {
      if (loggingEvent.level.isGreaterThanOrEqualTo(levelThreshold)) {
        appender.fileStream.write(layout(loggingEvent) + '\n', "utf8");
      }
    };
    appender.fileStream = fs.createWriteStream(options.filename, {
      flags: 'a',
      encoding: 'utf8',
      mode: 0644
    });
    appender.dispose = function() {
      if (appender.fileStream) {
        appender.fileStream.destroy();
      }
    };

    if (options.maxFileSize > 0) {
      setupLogRolling(appender, options.filename, options.maxFileSize, options.numBackups || 5, (options.filePollInterval * 1000) || 30000);
    }
...
function setupLogRolling(appender, filename, logSize, numBackups, filePollInterval) {
  fs.watchFile(filename,
    ...
    function(curr, prev) {
      if (curr.size >= logSize) {
        return rollThatLog(appender, filename, numBackups);
      }
    }
  );
}

function rollThatLog(appender, filename, numBackups) {
  //doing all of this fs stuff sync, because I don't want to lose any log events.
  //first close the current one.
  if (appender.fileStream) {
    appender.fileStream.on("close", function() {
      ...
      //open it up again for the new file
      appender.fileStream = fs.createWriteStream(filename, {
        flags: 'a',
        encoding: 'utf8',
        mode: 0644
      });
    });
    appender.fileStream.destroySoon();
  }
}
...
function replaceConsole(logger) {
  ...
  //wrap console output statements with our logger instead (taking care to preserve formatting)
  function replaceWith (fn) {
    return function() {
      var output = util.format.apply(util, arguments);
      fn.apply(logger, [output]);
    }
  }
...
['trace', 'debug', 'info', 'warn', 'error', 'fatal'].forEach(function(levelString) {
  ...
  _Logger.prototype[levelString] = function (message, exception) {
...
Logger.prototype = {
  ...
  isLevelEnabled: function(stringLevel, category) {
    var lvl = Level.toLevel(stringLevel);
    return getLogger(category).isLevelEnabled(lvl);
  },
  ...
  dispose: function() {
    for (var i = 0; i < appenders.length; i++) {
      if (appenders[i].dispose) {
        appenders[i].dispose();
      }
    }
    Logger.super.prototype.dispose.apply(this, arguments);
  }
