(function() {
   ...
   */
  jojo.widget.load = function(options) {
    var widgetClass = jojo.widget.loadedClasses.findById(options.path);
    ...
    jojo.socket.stateMachine.onceState("ready", function() {
      ...
      jojo.socket.sysChannel.once("loadwidget:" + id, function(args) {
        var cb = function() {
        ...
        args.result.widgetClasses.each(function(widgetClass) {
          if (!jojo.widget.loadedClasses.findById(widgetClass)) {
      ...
      jojo.socket.sysChannel.fire("loadwidget", {
        messageId: id,
        path: options.path,
