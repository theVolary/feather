...
function doRpc(client, message, cb) {
  cache.getItemWait("feather-logger", function(err, logger) {
    ...
    function e(ex) {
      logger.error({ message: "RPC error", category:"feather.socket", exception:ex });
      result.err = JSON.stringify(ex);
    ...
    Widget.loadClass(message.data.widgetPath, shortWidgetName, function(err, widgetClass) {
      if (err) e(err); else {
        ...
        message.data.params.push(function(err, ret) {
          ...
          //logger.debug({message: "instance is ${instance}", replacements: {instance: util.inspect(instance)}, category:"feather.sock"});
  ...
  cache.getItemsWait([
    ...
    if (err) cb(err); else {
      ...
      socketServer.sockets.on('close', function(errno) {
        logger.warn({message:"feather socket server shutting down.", category:'feather.srvr', immediately:true});
      });
