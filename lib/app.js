...
featherConfig.init({
  ...
  if (config.testing) {
    ...
    if (! err) {
        ...
        if (config.cluster && cluster.isMaster) {
          ...
          feather.start(config, function(err) {
            ...
            if (err) {
              ...
              //TODO: do we do anything with cluster.workers here?
            }
          });

        } else if (config.cluster) {
          
          feather.isWorker = true;
      ...
      // if we're running as a daemon, daemonize and then move on
      if (config.daemon.runAsDaemon && !process.env.IS_FEATHER_DAEMON) {
        console.log('daemonizing...');
        var daemon = require("daemon");
        daemon.daemonize(config.daemon.outputPath, config.daemon.pidPath, function(err, pid) {
          if (err) {
            console.error(util.inspect(err));
            process.nextTick(function() {
              process.exit(0);
