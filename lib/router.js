  ...
  cache.getItemsWait([
    ...
    if (err) cb(err); else {
      ...
      var router = connectRouter(function(app) {
        ...
        var handleFeatherReq = function(req, res, next) {
          ...
          if (appOptions.auth.rules) {
            for (var expression in appOptions.auth.rules) {
              ...
              // If verb values are provided, only match those specified.  If not proveded, all verbs are matched
              var verbMatch = !authRule.verbs || authRule.verbs.indexOf(req.method.toLowerCase()) > -1;
              if (verbMatch && req.url.match(authRule.regex)) {
        ...
        app.put(/\/(.*)/, handleFeatherReq);

        app.delete(/\/(.*)/, handleFeatherReq);
