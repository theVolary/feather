        ...
        var handleFeatherReq = function(req, res, next) {
          ...
          if (!redirect) {
            ...
            if (files.featherFiles[path]) {
              //TODO: for now, hardcoding the feather-res-cache path prefix, but at some point we might want to make that configurable...              
              var publishedPath = nodePath.join(options.publicRoot, "feather-res-cache", page);
              Connect.static.send(req, res, next, {path: publishedPath});
            } else {        
