...
exports.init = function(appOptions) {
  ...
  feather.auth.authInterface = Class.create(feather.event.eventPublisher, /** @lends feather.auth.authInterface.prototype */{
     ...
     * @returns undefined.  Result {@link feather.auth.userClass} is passed directly to client as the data property of the {@link feather.auth.resultClass} class via the client object (web socket)
     */
    login: function(options) {
      ...
      feather.logger.debug({message: "Getting user ${prefix}${user}", replacements:{prefix:feather.appOptions.auth.userIdPrefix, user:options.username}, category:'feather.auth'});
      feather.data.authdb.get(feather.appOptions.auth.userIdPrefix + options.username, function(err, dbResult) {
        if (!err) {
          ...
          if (dbResult.password === options.password) {
            ...
            feather.server.sessionStore.get(options.message.sid, function(err, sess) {
              sess.user = user;
              feather.server.sessionStore.set(options.message.sid, sess);
            });
          } else {
            feather.logger.warn("Invalid login attempt for username '" + options.username + "'");
    ...
    logout: function(options) {
      ...
      feather.request.session && feather.request.session.destroy();
    ...
    getCurrentUser: function(cb) {
      var user = feather.request.session ? feather.request.session.user : null;
      cb({user: user});
    }
  });
