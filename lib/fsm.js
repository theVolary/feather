(function() {    
    ...
    jojo.fsm.finiteStateMachine = Class.create(jojo.event.eventPublisher, {
        ...
        fsmEvent: function(eventName, args) {            
            if (this.currentState[eventName] || this.states.global[eventName]) {
                ...
                if (newState && newState !== this.currentState) {
                    ...
                    this.fire("leavingState", {currentState: this.currentState, newState: newState});
        ...
        gotoState: function(state, eventName, eventArgs) {
            ...
            //        }
            //    };    
            this.fire("stateStartup", {eventName: eventName, eventArgs: eventArgs});
        },
    ...
    jojo.stateMachine = new jojo.fsm.finiteStateMachine({
        states: {
            ...
            ready: {
                //TODO: what global operations or other things might be needed
                //testing...
                stateStartup: function(fsm, args){
                    require("sys").puts("in jojo.stateMachine.ready state...");
                }
            }
        }
    });
