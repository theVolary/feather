    ...
    '{{if options}}',
      '<options>',
        '{{each(i, option) options}}',
          '<option name="${name}" value="{{html value}}" {{if client_enabled}}client_enabled="${client_enabled}" {{/if}}/>',
        '{{/each}}',
      '</options>',
    '{{/if}}',
    '{{if contentTemplate}}',
      ...
      function(err, cacheItems) {
        ...
        function render() {
          if (me.container && args) {
            ...
            if (me.preserveTemplateNewlines) {
              me.template = me.template.replace(/\n/g, '{{html newline}}');
              me.newline = '\\n';
            }

            try {
              var _t = $j.template(null, me.template);
              var _html = _t($j, {data: me}).join("");
              me.container.html(_html);
            } catch(templateException) {
              me.fire('error', {err: "Error rendering template: " + JSON.stringify(templateException)}, cb)
            }
        ...
        if (typeof me.onRender === "function") {
          ...
          var timer = setTimeout(function(){
            if(!renderCalled) {
              logger.warn({
                ...
                message: "Render timed out for widget " + me.widgetPath
              });
              render();
            }
          }, me.options.renderTimeout || appOptions.renderTimeout || 20000);
          render = function() {
            clearTimeout(timer);
            if(!renderCalled) {
              renderCalled = true;
          ...
          try {
            ...
            //onRender had an uncaught exception - log and render anyway, so we don't eat up the server dom
            logger.error({
              category: "widget.render",
              message: "Error in onRender method",
              exception: e
            });
            render();
          }
...
Widget.registerTagHandler = function(tagName, tagRenderer, disallowedTags) {
  try {
    ...
    if (tagBlacklist.indexOf(tagName.toLowerCase()) != -1) {
      throw new Error('"' + tagName + '" is reserved for internal use and cannot have a custom handler registered.');
    }
    ...
    throw new Error("Cannot add a tag handler for '" + tagName + "'. That tag name is reserved for internal use or already has a handler registered.");
  }
};
...
Widget.registerTagHandler("template", function(options) {
  ...
  if ($j(options.tag).parent()[0] !== options.widget.container[0]) {
    throw new Error("<template> tags may only be defined at the top level within a [widget].template.html file (i.e. they cannot be embedded within other tags). id=" + options.widget.myid);
  }
  ...
  if (client_enabled == "true" && !options.widget.serverOnly) {
    var clientTemplate = {
      id: name,
      tmpl: tmpl
    };
...
}, ["template", "widget"]); //disallowed tags
...
Widget.registerTagHandler("insert_template", function(options) {
  ...
  $j(options.tag).remove();
}, ["template", "widget", "insert_template"]); //disallowed tags
 ...
 * @static
 */
Widget.loadClass = function(options, cb) {
  cache.getItemsWait([
    "feather-options",
    "feather-files"
  ], function(err, cacheItems) {
    if (err) cb(err); else {
      var appOptions  = cacheItems["feather-options"],
        ...
        widgetPath    = nodePath.join(appOptions.publicRoot, path, widgetName);

      if (!widgetClass || options.forceReparse) { //this class has not yet been defined or needs to be redefined, so load the class file now
        var filePath = widgetPath + ".server";
        if (!fs.existsSync(filePath + ".js")) {
          filePath = widgetPath + ".client.js";
          ...
          if (typeof getWidget !== "function") {
            ...
            getWidget(require("./feather"), function(err, widgetDef) {
              if (err) cb(err); else {
                ...
                widgetClass.fsWidgetPath      = widgetPath;
                widgetClass.clientFilePath    = widgetPath + ".client.js";
                widgetClass.widgetRoot        = "/" + path;
                widgetClass.clientHrefPath    = "/" + path + widgetName + ".client.js";
                widgetClass.templateFilePath  = widgetPath + ".template.html";
                widgetClass.template          = files.templateFiles[widgetClass.templateFilePath].data || " ";
                widgetClass.clientCssFilePath = widgetPath + ".css";
...
Widget.render = function(renderOptions, callback) {
  ...
  var result            = renderOptions.result,
    ...
    renderTimeout       = renderOptions.renderTimeout;
    ...
    $j('widget:first[clientonly!="true"]', node).each(function(index, widgetEl) {
      ...
      if (!pathError) {
        Widget.loadClass({
          path: path,
          widgetName: widgetName,
          forceReparse: renderOptions.forceReparse || renderOptions.appOptions.developer.watchForChanges
...
Widget.doRpc = function doRpc(request, client, data, cb) {
  ...
  Widget.loadClass({
      path: data.widgetPath,
      widgetName: shortWidgetName
    }, function(err, widgetClass) {
