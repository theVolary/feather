...
Widget.registerTagHandler("template", function(options) {
  ...
  if ($j(options.tag).parent()[0] !== options.widget.container[0]) {
    throw new Error("Template tags may only be defined at the top level within a <widget>.template.html file (i.e. they cannot be embedded).");
  }
    ...
    if (err) cb(err); else {
      ...
      if (!widgetClass) { //this class has not yet been defined, so load the class file now
        ...
        if (typeof getWidget !== "function") {
          ...
          getWidget(appOptions, function(err, widgetDef) {
            if (err) cb(err); else {
...
Widget.getInstance = function(options, cb) {
  var parts = options.widgetName.split(".");
  Widget.loadClass(options.widgetPath, parts[parts.length - 1], function(err, widgetClass) {
    if (err) cb(err); else {
      if (widgetClass) {
        cb(null, new (widgetClass.classDef)(options));
      } else {
        cb();
      }
    }
...
Widget.render = function(renderOptions, callback) { 
  ...
  $j("widget:first", node).each(function(index, widgetEl) { 
    ...
    Widget.loadClass(path, widgetName, function(err, widgetClass) {
      if (err) callback(err);
      if (!widgetClass) {
        callback(new Error("widgetClass is undefined for path '" + path + "'"));
      }
      
      //dump the widget class into the registry to track unique widget classes used
      if (widgetClassRegistry && !widgetClassRegistry.findById(widgetClass.path)) {
...
Widget.create = function(options) {
  if (!options || !options.path || !options.name || !options.prototype) {
    throw new Error("Widget.create requires an options argument with 'path', 'name' and 'prototype' properties.");
  }
