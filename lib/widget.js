  ...
  cache.getItemsWait([
    ...
    if (err) cb(err); else {
      var appOptions  = cacheItems["feather-options"],
        ...
        fileMeta      = files.templateFiles[widgetPath + '.template.html'];

      if (!widgetClass || (fileMeta && fileMeta.needsReload)) { //this class has not yet been defined or needs to be redefined, so load the class file now
