...
var importExportStates = {
  initial: {
    verifyPaths: function() {
      return this.states.verifyingDataFolder;
    }
  },
  verifyingDataFolder: {
    stateStartup: function(args) {
      var me = this;
      path.exists(me.dataFolder, function(exists) {
        if (!exists) {
          me.fire('createFolder', args);
        } else {
          me.fire('pathVerified', args);
        }
      });
    },
    createFolder: function(args) {
      var me = this;
      fs.mkdir(me.dataFolder, '0755', function(err) {
        if (err) { me.fire('error', { err: err }); return; }
        me.fire('pathVerified', args);
      })
    },
    pathVerified: function(args) {
      return this.states.verifyingSubFolders;
    }
  }, // end verifyingDataFolder state
  verifyingSubFolders: {
    stateStartup: function(args) {
      var me = this;
      me.subpathSemaphore = new Semaphore(function() {
        return me.states.subfoldersVerified;
      });
      me.fire('verifyFolder', { path: me.designFolder });
      me.fire('verifyFolder', { path: me.docFolder });
    },
    verifyFolder: function(args) {
      var me = this;
      me.subpathSemaphore.increment();
    ...
    createFolder: function(args) {
      ...
      fs.mkdir(args.path, '0755', function(err) {
        if (err) me.fire('error', { err: err }); else {
         me.fire('pathVerified', args);
        }
      })
    },
    pathVerified: function(args) {
      return this.subpathSemaphore.execute();
    }
  }, // end verifyingSubFolders
  error: {
    stateStartup: function(args) {
      if (this.finalCallback) this.finalCallback(args.err);
    }
  }, // end aborted
};
