...
dataInterface.prototype = {
    ...
    if (!docOrDocArray) {
      ...
      if (docId) {
        if (docOrDocArray._rev) {
          this.db.save(docId, docOrDocArray._rev, docOrDocArray, options.couch, saveCallback);
        } else {
          this.db.save(docId, docOrDocArray, options.couch, saveCallback);
        }
      } else {
        this.db.save(docOrDocArray, options.couch, saveCallback);
      }
    }
  },
  ...
  filterDocuments: function(options, cb) {

    // skippity zippah!
    if (! options.docs.length) return cb(null, options.docs);

    var me = this,
        docs = options.docs,
        filter = parseFilter(options.filter).props,
        newDocs = [],
        filterErrors = [],
        fdSem = new Semaphore(function() {
          if (!filterErrors.length) {
            filterErrors = null;
          } else {
            filterErrors = JSON.stringify(filterErrors);
          }

          // sort, then blow away our temp sorting property.
          newDocs = _.sortBy(newDocs, function(doc) {
            return doc._feather_sort_index;
          });
          _.each(newDocs, function(doc) {
            delete doc._feather_sort_index;
          });
          cb(filterErrors, newDocs);
        });

    var filterDoc = function(doc, docFilter, docCb) {
      var newDoc = {},
          errors = [],
      ...
      sem.increment();
      _.each(docFilter, function(prop) {

        sem.increment();
        if (typeof prop === 'string') {
          newDoc[prop] = doc[prop];
          sem.execute();
        } else if (typeof prop === 'object' && ((doc[prop.name] && doc[prop.name]['$ref']) || _.isArray(doc[prop.name]))) { 
          // object ref, meaning it's a referenced doc we need to go get and pull props off of.
          var toFilter = doc[prop.name];
          if (! _.isArray(toFilter)) {
            toFilter = [toFilter];
          } else {
            newDoc[prop.name] = [];
          }
          var childrenDoneSem = new Semaphore(function() {
            if (_.isArray(newDoc[prop.name])) {
              var sortedArray = [], foundItem;
              _.each(toFilter, function(refDoc) {
                foundItem = _.find(newDoc[prop.name], function(arrayItem) {
                  return refDoc['$ref'] === arrayItem['_$origRef'];
                });
                if (foundItem) {
                  sortedArray.push(foundItem);
                }
              });
              newDoc[prop.name] = sortedArray;
            }
          });
          _.each(toFilter, function(docToFilter) {

            childrenDoneSem.increment();
            sem.increment();
            me.get({ id: docToFilter['$ref'] }, function(refErr, refDoc) {
              if (refErr) {
                errors.push(prop.name + ': ' + docToFilter['$ref'] + ' error: ' + refErr.message);
              } else {
                sem.increment();
                childrenDoneSem.increment();
                filterDoc(refDoc, prop.properties, function(sdErr, subDoc) {
                  if (sdErr) {
                    errors.push(prop.name + ': ' + docToFilter['$ref'] + ' error: ' + sdErr.message);
                  } else {
                    subDoc['_$origRef'] = docToFilter['$ref'];
                    if (newDoc[prop.name] && _.isArray(newDoc[prop.name])) {
                      newDoc[prop.name].push(subDoc);
                    } else {
                      newDoc[prop.name] = subDoc;
                    }
                  }
                  childrenDoneSem.execute();
                  sem.execute();
                });
              }
              sem.execute();
    ...
    _.each(docs, function(doc, index) {
      fdSem.increment();
      filterDoc(doc, filter, function(docErrors, newDoc) {
        if (docErrors && docErrors.length) { 
          _.each(docErrors, function(filErr) {
            filterErrors.push(filErr);
          });
        } else {
          newDoc._feather_sort_index = index;
          newDocs.push(newDoc);
        }
        fdSem.execute();
      })
  ...
  find: function(options, cb) {
    ...
    var fetchPage = function() {
      me.db.view(options.source, findOptions, function(err, documents) {
        if (err) { 
          ...
          if (options.filter) {
            sem.increment();
            me.filterDocuments({docs: documents, filter: options.filter}, function(filterErr, filteredDocs) {
              if (filterErr) {
                cbError = filterErr;
                cbResult = null;
              } else {
                cbError = null;
                cbResult.documents = filteredDocs;
              }
              sem.execute();
            });
          }
    ...
    if (paginationOn) {
      ...
      me.db.view(options.source, totalFindOptions, function(err, count) {
        if (err) { 
          if (!prefetchBoundaries) { // prefetchBoundaries needs to be false to set to 0 or we will have a race condition that could clobber the data
      ...
      if (pagination.cachePageBoundaries) {
        ...
        if (alreadyHasBoundaries) {
          if (pagination.pageBoundaries.length > pageNum) {
            if (findOptions.keys) { // handle find options based on list of keys  

              findOptions.keys = _.rest(findOptions.keys, findOptions.keys.indexOf(pagination.pageBoundaries[pageNum].key)); // advance the keys (this accounts for skipped rows for unknown keys)
            } else { // handle find options using a startkey

                findOptions.startkey = pagination.pageBoundaries[pageNum].key;
                if (pagination.pageBoundaries[pageNum]._id) {
      ...
      if (prefetchBoundaries) {
        ...
        // setup prefetch based on query being used
        if (prefetchOptions.keys) {
          prefetchOptions.criteriaType = 'byKeys';
          delete prefetchOptions.include_docs;
          if (_.unique(prefetchOptions.keys).length !== prefetchOptions.keys.length) {
            internalCb(formatError("Pagination + Non-unique list of Keys = Bad Stuff"));
            return;
          }
        } else if (prefetchOptions.startkey) {
          prefetchOptions.criteriaType = 'byStartKey';
          delete prefetchOptions.include_docs;
        } else if (prefetchOptions.key) {
          prefetchOptions.criteriaType = 'byKey';
          delete prefetchOptions.include_docs;
        } else {
          delete prefetchOptions.include_docs;
        }
        ...
        var prefetchSem = new Semaphore(function() {
          if (cbError) {
            ...
            pagination.docCount = runningDocCount;
            findOptions.limit = pagination.pageSize;
            fetchPage();
          }
        });
        ...
        var handlePageBoundary = function(err, documents) {
          if (err) {
            ...
            if (documents.length <= pagination.pageSize) { // since documents.length is supposed to be pagesize+1, we are at the end when it equals pagesize
              if (documents.length) {
              ...
              if (prefetchOptions.criteriaType === 'byKeys') { 
                pagination.pageBoundaries.push({key: documents[0].key, _id: documents[0].id});
                prefetchOptions.keys = _.rest(prefetchOptions.keys, prefetchOptions.keys.indexOf(documents[documents.length-1].key)); // advance the keys (this accounts for skipped rows for unknown keys)
              } else if (prefetchOptions.criteriaType === 'byStartKey' || !prefetchOptions.criteriaType) {
                pagination.pageBoundaries.push({key: documents[0].key, _id: documents[0].id});
                prefetchOptions.startkey = documents[documents.length-1].key;
                prefetchOptions.startkey_docid = documents[documents.length-1].id;
              } else { // just a search byKey
                pagination.pageBoundaries.push({key: documents[0].key, _id: documents[0].id});
              }

              //need to always use original key because of problems with too many escape characters being added when key is a string
              prefetchOptions.key = findOptions.key;
              if (typeof prefetchOptions.key == 'undefined') delete prefetchOptions.key;            
              me.db.view(options.source, _.clone(prefetchOptions), handlePageBoundary);
            }
          }
        }
...
var parseFilter = function(filter) {
  ...
  for (tokenCount = 0; tokenCount < filter.length; tokenCount++) {
    currToken = filter[tokenCount];
    if (currToken === ',') {
      ...
      var subProps = parseFilter(filter.substring(++tokenCount));
      currProp = {
        name: currProp,
        properties: subProps.props
      };
      tokenCount += subProps.tokenCount;
      props.push(currProp);
      currProp = '';
  ...
  if (typeof currProp === 'object' || currProp.length) {
    props.push(currProp);
  }
  return { props: props, tokenCount: tokenCount };
};
