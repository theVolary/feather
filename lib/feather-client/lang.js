...
(function() { //module pattern for client-safety, as this code could run on either the client or the server
    ...
    //lightweight universal object cache for all feather objects
    feather.lang.objects = {};
    ...
    /**
     * The lowest level for all feather objects (namely widgets, etc) (require unique id, etc.)
     * @param {Object} options
     */
    feather.lang.baseClass = Class.create({
        initialize: function(options) {
            options = options || {};
            this.id = options.id || feather.id();
            if (feather.lang.objects[this.id]) {
                throw new Error("All feather base objects must have a unique .id property specified. ID: '" + this.id + "'");
            }
            feather.lang.objects[this.id] = this;
        },
        dispose: function() {
            delete feather.lang.objects[this.id];
            for (var p in this) {
                delete this[p];
            }
        }
     ...
     */
    feather.lang.registry = Class.create(feather.lang.baseClass, {
        /**
     ...
     */
    feather.lang.semaphore = Class.create({
