(function() {
  ...
  var Widget = feather.Widget = function(options) {
    ...
    } else if (this.containerizer || this.containerOptions) {
      var containerizer = this.containerizer || this.containerOptions.containerizer;
    ...
    //load css if needed
    if (! options.compileToSingleCss && ! this.widgetPath && !feather.Widget.resources.findById(this.widgetPath + ".css")) {
    ...
    datalink: function(options) {
      ...
      me.get("form").each(function(index, form) {
        ...
        if (datalink) {
          ...
          //first, set field values based on existing model properties
          for (var p in me.model[datalink]) {
            $("[name=" + p + "]", form).each(function(index, field) {
              field = $(field);
              if (field.attr('type') === "radio") {
                if (me.model[datalink][p] === field.val()) {
                  field.attr('checked', "checked");
                }
              } else if (field.attr('type') === "checkbox") {
                if (me.model[datalink][p] === true) {
                  field.attr('checked', "checked");
                } else {
          ...
          //now, trigger change on all the form fields and fix radios and checkboxes
          //end result should be in sync model and fields
          $('input', form).each(function(index, field) {
            field = $(field);
            ...
            if (type === "radio") {
              ...
              if (typeof me.model[datalink][name] === 'undefined') {
                //prepopulate model if field is pre-checked
                var checked = field.attr('checked') == 'checked';
                if (checked) me.model[datalink][name] = field.val();
                return;
              }

              if (me.model[datalink][name] === field.val()) {
                field.attr('checked', "checked");
              }
              ...
              if (typeof me.model[datalink][name] === 'undefined') {
                //prepopulate model if field is pre-checked
                var checked = field.attr('checked') == 'checked';
                me.model[datalink][name] = checked;
                return;
              }

              if (me.model[datalink][name] === true) {
                field.attr('checked', "checked");
              } else {
                field.removeAttr('checked');
              }    

              //manually wireup datalinking for checkboxes
              me.domEvents.bind(field, 'click', function() {
                if (field.attr('checked') === 'checked') {
                  me.model[datalink][name] = true;
                } else {
                  me.model[datalink][name] = false;
                }
              });            
            } else {
      ...
      stateStartup: function(args, cb) {
        ...
        if (classObj.clientOnly) {
          if (classObj.template) {
            var me = this;
            me.template = me.template || classObj.template;
...
//TODO: Refactor this stuff to live in a singly maintained location shared between server and client parsers
  ...
  Widget.render = function(renderOptions, callback) { 
    ...
    widgetSelector               = renderOptions.widgetSelector || widgetSelector;
    optionSelector               = renderOptions.optionSelector || optionSelector;
    optionsSelector              = renderOptions.optionsSelector || optionsSelector;
    contentSelector              = renderOptions.contentSelector || contentSelector;
    contentTemplateSelector      = renderOptions.contentTemplateSelector || contentTemplateSelector;
     ...
     */
    // setTimeout(function() { //break out of the callstack (prevents call stack overflows when dealing with large nested structures)
            ...
            Widget.loadClass(path, widgetName, function(err, widgetClass) {
              if (err) callback(err); else {                              
                ...
                if (!widgetClass.clientOnly) {
                  ...
                  //recurse down the children tree first so rendering goes from the bottom 
        ...
        var cb = function() {
          ...
          if (widgetClass && widgetClass.clientOnly) {
            ...
            if (widgetClass) {
              ...
              //serverOnly case
              options.clientOptions.html = fixXMLNS(args.result.html, true);
              var widget = new feather.Widget(options.clientOptions);
              widget.fire('init');
              widget.fire('ready');
            }
          }
