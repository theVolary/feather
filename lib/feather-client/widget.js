          ...
          if (classObj.template) {
            ...
            if (me.container /*&& args*/) {
              ...
              if (me.contentTemplate) {
                if (typeof me.contentTemplate === "string") me.contentTemplate = $j('<contentTemplate>' + me.contentTemplate + '</contentTemplate>');
                var tmpl = $j.tmpl(me.contentTemplate.html(), me);
                if (tmpl && tmpl.appendTo) {
                  if ($j("content", me.container).length) {
                    $j("content", me.container).each(function(index, content){
              ...
              if (!widgetClass.clientOnly) {
                ...
                Widget.render(_.extend(_.clone(renderOptions), {
                  ...
                  if (err) callback(err); else {
                    ...
                    if (typeof instance.contentTemplate === "string") instance.contentTemplate = $j('<contentTemplate>' + instance.contentTemplate + '</contentTemplate>');
