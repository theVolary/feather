(function() {
  ...
  var contentTagFixRegex = /<(\/*)content[^a-zA-Z]/g;
  var contentTemplateTagFixRegex = /<(\/*)contentTemplate/g;
  var optionsSelector = $.browser.msie ? "options" : "feather\\:options";
  var contentTemplateSelector = $.browser.msie ? "contentTemplate" : "feather\\:contentTemplate";
  var contentSelector = $.browser.msie ? "content" : "feather\\:content";
  ...
  var Widget = feather.Widget = function(options) {
    ...
    if (typeof this.contentTemplate === "string") {
      this.contentTemplate = this.contentTemplate.replace(idFixRegex, "<$1 $3${id}_$4$5>");
      this.contentTemplate = this.contentTemplate.replace(widgetTagFixRegex, "<$1feather:widget");
      this.contentTemplate = this.contentTemplate.replace(optionsTagFixRegex, "<$1feather:options");
      this.contentTemplate = this.contentTemplate.replace(contentTemplateTagFixRegex, "<$1feather:contentTemplate");
      this.contentTemplate = this.contentTemplate.replace(contentTagFixRegex, "<$1feather:content");
    }
      ...
      '{{if options}}',
        '<feather:options>',
          '{{each(i, option) options}}',
            '<option name="${name}" value="{{html value}}" />',
          '{{/each}}',
        '</feather:options>',
      '{{/if}}',
      '{{if contentTemplate}}',
...
//TODO: Refactor this stuff to live in a singly maintained location shared between server and client parsers
  ...
  Widget.create = function(options) {
    ...
    if (!classObj) {
      ...
      if (classObj.template) {
        classObj.template = classObj.template.replace(idFixRegex, "<$1 $3${id}_$4$5>");
        classObj.template = classObj.template.replace(widgetTagFixRegex, "<$1feather:widget");
        classObj.template = classObj.template.replace(optionsTagFixRegex, "<$1feather:options");
        classObj.template = classObj.template.replace(contentTemplateTagFixRegex, "<$1feather:contentTemplate");
        classObj.template = classObj.template.replace(contentTagFixRegex, "<$1feather:content");
      }
      if (options.inherits) {
        var inheritedClass = feather.Widget.loadedClasses.findById(options.inherits.prototype.widgetPath);
        if (inheritedClass) {
          classObj.template = classObj.templace || inheritedClass.template;
        }
      }
        ...
        feather.socket.sysBus.once("loadwidget:" + messageId, function(args) {
          var cb = function() {
            ...
            if (widgetClass && widgetClass.clientOnly) {
              ...
              options.clientOptions.html = args.result.html;
              options.clientOptions.html = options.clientOptions.html.replace(widgetTagFixRegex, "<$1feather:widget");
              options.clientOptions.html = options.clientOptions.html.replace(optionsTagFixRegex, "<$1feather:options");
              options.clientOptions.html = options.clientOptions.html.replace(contentTemplateTagFixRegex, "<$1feather:contentTemplate");
              options.clientOptions.html = options.clientOptions.html.replace(contentTagFixRegex, "<$1feather:content");
