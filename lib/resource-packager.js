...
exports.packageResources = function(options, callback) {
  simpleCache.getItem("feather-logger", function(err, logger) {
    ...
    var whenAllComponentsAdded = new Semaphore(function() {
    ...
    whenAllComponentsAdded.setDebug(true, "whenAllComponentsAdded " + options.cacheName);
        ...
        if (newCache) {
          fs.readFile(filePath, "utf8", function(err, content) {
            ...
            if (err) {
              ...
              whenAllComponentsAdded.increment();
              cache.addComponent(filePath, content, function(err) {
                whenAllComponentsAdded.execute();
              });
            }
          });
        }
    ...
    if (!newCache) {
      logger.debug({message:"Finished packaging existing cache " + cache.id, category:"feather.respack"});
      whenAllComponentsAdded.increment();
      whenAllComponentsAdded.execute();
    }
  });
...
exports.packageFrameworkResources = function(options, callback) {
  ...
  var frameworkPackaged = new Semaphore(function() {
    callback();
  });
  frameworkPackaged.setDebug(true, "frameworkPackaged");
  
  frameworkPackaged.increment();
  exports.packageResources(jsOptions, function() {
  ...
  if (cssFiles.length) {
    ...
    frameworkPackaged.increment();
    exports.packageResources(cssOptions, function() {
      frameworkPackaged.execute();
    });
  }
};
...
exports.packageWidgetResources = function(options, callback) {
  ...
  widgetResourcesPackaged.setDebug(true, "widgetResourcesPackaged " + options.cacheName);
