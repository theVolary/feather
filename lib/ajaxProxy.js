...
module.exports = function(req, res, next) {
  ...
  var bodySid = req.body.sid;

  cache.getItem('feather-options', function(err, appOptions) {

    bodySid = featherUtil.parseSignedCookie(bodySid, appOptions.connect.session.secret);

    //make sure message sid matches server sid
    if (!bodySid || bodySid !== req.sessionId) {
