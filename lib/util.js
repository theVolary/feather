...
module.exports = /** @lends Util.prototype */ {
      ...
      while (parts.length > 0) {
        ...
        if (curr != '/') {
          try {
            ...
            if (ex.errno != -17) { // File already exists - no problem
  ...
  parseSignedCookies: function(cookies, secret) {
    if (! cookies) return null;
    return Connect.utils.parseSignedCookies(cookies, secret); 
  },

  parseSignedSessionId: function(signedSessionId, cb) {
    if (! signedSessionId) cb(null, null);
    var me = this;
    cache.getItemWait('feather-options', function(err, appOptions) {
      cb(null, me.parseSignedCookie(signedSessionId, appOptions.connect.session.secret));
    });
  }
};
