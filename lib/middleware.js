          ...
          function completeMiddleware() {
            ...
            cache.getItem("feather-logger", function(err, logger) {
              if (err) {
                ...
                if (appOptions.connect.timing.enabled === true) {
                  logger.info('Enabling request timing logs');
                  middleware.unshift(function(req, res, next) {
                    var prevEnd = res.end;
                    res.end = function(chunk, encoding) {
                      res.end = prevEnd;
                      res.end(chunk, encoding);
                      var now = new Date(),
                          duration = now - req.timestamp.start;
                      if (duration >= appOptions.connect.timing.logging.thresholdMs) {
                        var url = nodeUrl.parse(req.url);
                        logger.info({
                          category: appOptions.connect.timing.logging.category,
                          message: '${method},"${url}","${query}","${body}",${start},${end},${duration}',
                          replacements: {
                            method: req.method,
                            url: url.pathname,
                            start: req.timestamp.start.toString(),
                            end: now.toString(),
                            duration: duration,
                            query: url.query,
