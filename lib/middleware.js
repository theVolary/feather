...
exports.middleware = [
  ...
  Connect.session(jojo.appOptions.session.config),
  function(req, res, next) {
    //easy access to current request and response
    jojo.request = req;
    jojo.response = res;
    if (!jojo.server.sessionStore) {
      jojo.server.sessionStore = req.sessionStore;
    }
    jojo.stateMachine.fire("request", {request: req, response: res, next: next});
  },
  // jojo framework level routers (socket io, resource packager stuff, etc.)
