...
exports.init = function(options, cb) {
  ...
  var fsm = new FSM({
    states: {
      ...
      createServer: {
        stateStartup: function() {
          middleware.getMiddleware(options, function(err, _middleware) {
            if (err) fsm.fire("error", err); else {
              var mirror = null,
                sem = 0;
              ...
              var completeServerSetup = function() {
                ...
                var server = Connect.apply(this, _middleware);
              ...
              if (options.ssl && options.ssl.enabled) {
                ...
                if (options.ssl.useRedirectServer && !options.ssl.routes) {
                  ...
                  sem++;
                  middleware.getMiddleware(options, function(err, __middleware) {
                    ...
                    mirror = Connect.apply(this, __middleware);
