...
var idFixRegex = /<(([^w\/\s]|w[^i\s]|wi[^d\s]|wid[^g\s]|widg[^e\s]|widge[^t\s])+)([^><]*\sid=['"])([^'"\$]*)(['"][^>]*)>/g;
          ...
          if (options.isMaster && options.developer.watchForChanges) {
            ...
            watch.createMonitor(options.publicRoot, {
              ...
              filter: function(filePath) {
                ...
                if (ret) console.log("including " + filePath + " in watch list.");
                return ret;
              }
            }, function(monitor) {
              console.log("Watching feather files and templates for changes.");
              var processChange = function(filename, stat) {
                if (filename.indexOf('feather-res-cache') === -1 && !!filename.match(/\.(feather|template)\.html$/)) {
                  console.log("Processing " + filename + " due to a change");
                  cache.getItemWait("feather-files", function(err, indexedFiles) {
                    if (err) console.log("Couldn't get feather-files from cache: " + err); else {
                              ...
                              process.nextTick(function() {
                                parser.parseFile({
                                  ...
                                  request: {page: filename.replace(/.*\/public\/(.*)$/, "$1")} //need a dummy request object for parser since there is no real request at this point
                                    ...
                                    if (err) console.log(err); else console.log("Re-parsed " + featherFilePath + " due to a change in " + filename);
                                });
                              });
                            }
                          });
                        ...
                        process.nextTick(function() {
                          parser.parseFile({
                            ...
                            request: {page: filename.replace(/.*\/public\/(.*)$/, "$1")} //need a dummy request object for parser since there is no real request at this point
                              ...
                              if (err) console.log(err); else console.log("Re-parsed " + filename);
                          });
                        });
                      }
                    }
