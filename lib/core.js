exports.bootstrap = function(ctx) {
  ...
  context.feather = /** @lends feather */ {
      ...
      /**
       * @memberOf feather
       * @description Simple incremental ID provider for cases where the developer wants to rely on the framework to auto ID stuff
      ...
      shutdown: function() {
        if (feather.stateMachine) {
          feather.stateMachine.onceState("ready", function() {
            if (feather.server) {
              feather.logger.info({message:"feather server shutting down.", category:'feather.srvr'});
