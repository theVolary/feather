...
module.exports = {
  ...
  publish: function(options, publisher, cb) {
    sc.getItems([
      ...
      if (publisherOptions.consolidate) {
        logger.debug({ 
        ...
        s3Client.putBuffer(new Buffer(publisher.concat(), 'utf-8'), publisher.id, headers, function(err, res) {
          if (err) {
            logger.error({
        ...
        _.each(publisher.components, function(component, key) {
          logger.debug({
            message: "Publishing ${relativePath} to ${url}",
            category: "feather.publisher.s3",
            replacements: component
          });
          componentPubSem.increment();
          s3Client.putBuffer(new Buffer(component.content, 'utf-8'), component.relativePath, headers, function(err, res) {
