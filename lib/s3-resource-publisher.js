...
module.exports = {
  ...
  publish: function(options, publisher, cb) {
    sc.getItems([
      ...
      var baseUrl = publisherOptions.config.publishUri || s3Client.url('') + publisherOptions.config.bucketFolder;
      ...
      if (packageOptions.consolidate) {
        ...
        logger.debug({ 
          ...
          replacements: { id: publisher.id, url: baseUrl + separator + publisher.id }, 
          category: 'feather.publisher.s3' });
        publishResult.consolidatedUrl = baseUrl + separator + publisher.id;
