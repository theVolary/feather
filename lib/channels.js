...
Channel.prototype = {
      ...
      me.doHook("subscribe", hookArgs, function(err, hookData) {
        if (err && err !== nohook) {
          ...
          if (added) {            
            ...
            me.clientsBySessionId[client.session.id] = me.clientsBySessionId[client.session.id] || [];
            me.clientsBySessionId[client.session.id].push(client);
  ...
  resolveClient: function(_client) {
    ...
    if (client) {
      ...
      var clients = this.clientsBySessionId[_client];
      if (clients) {
        return clients;
      }
    }
